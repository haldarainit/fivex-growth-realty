...
export const TeamCard: React.FC<TeamCardProps> = ({ member }) => {
  return (
    <div className="group">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
          <div className="flex gap-4">
            <button aria-label="Share profile" className="cursor-pointer">
              <span className="material-symbols-outlined text-white hover:text-secondary transition-colors">
                share
              </span>
            </button>
      ...
      <p className="font-body text-xs text-secondary font-bold uppercase tracking-wider">
        {member.role}
      </p>
    </div>
  );
